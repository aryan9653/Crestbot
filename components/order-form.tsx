...
export function OrderForm({ market = "CRYPTO", broker = "Zerodha" }: { market?: MarketKind; broker?: IndiaBroker }) {
  ...
  // India
  const [iSymbol, setISymbol] = useState<string>("RELIANCE")
  ...
  const onPlace = async (side: "BUY" | "SELL") => {
    ...
    const payload =
      segment === "Equity"
        ...
        ? {
            ...
            segment,
            product,
            tradingsymbol: `${iSymbol}_${expiry}_FUT`,
        ...
        : {
            ...
            segment,
            product,
            tradingsymbol: `${iSymbol}_${expiry}_${strike}${optType}`,
            quantity: lots * lotSize,
            orderType: "MARKET",
          }
    ...
    try {
      ...
      alert("Broker order response (mock):\n" + JSON.stringify(json, null, 2))
    } catch (e: any) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{market === "CRYPTO" ? "Advanced Order (Crypto)" : `Order (India • ${segment} • ${broker})`}</CardTitle>
      </CardHeader>
      ...
      {market === "CRYPTO" ? (
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>Symbol</Label>
              <Select value={cSymbol} onValueChange={setCSymbol}>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>Symbol</Label>
              <Select value={iSymbol} onValueChange={setISymbol}>
                <SelectTrigger className="mt-1.5"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {["RELIANCE", "TCS", "HDFCBANK", "INFY", "SBIN", "NIFTY", "BANKNIFTY"].map((s) => (
                    <SelectItem key={s} value={s}>{s}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          {segment !== "Equity" && (
            <div className="grid grid-cols-3 gap-3">
              <div>
                <Label>Expiry</Label>
                <Input type="date" value={expiry} onChange={(e) => setExpiry(e.target.value)} className="mt-1.5" />
              </div>
              {segment === "Options" && (
                <>
                  <div>
                    <Label>Strike</Label>
                    <Input type="number" value={strike} onChange={(e) => setStrike(parseInt(e.target.value || "0"))} className="mt-1.5" />
                  </div>
                  <div>
                    <Label>Type</Label>
              ...
              <div className={cn("col-span-1", segment === "Options" ? "" : "col-start-3")}>
                ...
                <Input type="number" value={lots} min={1} onChange={(e) => setLots(Math.max(1, parseInt(e.target.value || "1")))} className="mt-1.5" />
              </div>
            </div>
          )}
